<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50">
    <div class="max-w-md w-full space-y-8 text-center">
      ...
      <div v-else-if="error">
        <div
          class="rounded-full h-12 w-12 bg-red-100 flex items-center justify-center mx-auto"
        >
          <UIcon name="i-heroicons-x-mark" class="h-6 w-6 text-red-600" />
        </div>
        <h2 class="mt-4 text-xl font-semibold text-gray-900">Sign in failed</h2>
        <p class="mt-2 text-sm text-gray-600">
          {{ error.message }}
        </p>
        <UButton to="/auth/login" class="mt-4"> Try again </UButton>
      </div>

      <div v-else>
        <div
          class="rounded-full h-12 w-12 bg-green-100 flex items-center justify-center mx-auto"
        >
        ...
        <UButton to="/" class="mt-4"> Go to Dashboard </UButton>
      </div>
    </div>
  </div>
</template>
