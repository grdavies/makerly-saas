      ...
      <div class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
        <UCard class="max-w-md mx-4">
          <div class="text-center space-y-4">
            <div class="mx-auto w-12 h-12 bg-orange-100 dark:bg-orange-900/20 rounded-full flex items-center justify-center">
              <UIcon name="i-heroicons-lock-closed" class="w-6 h-6 text-orange-600 dark:text-orange-400" />
            </div>
            ...
            <!-- Usage info -->
            <div v-if="showUsageInfo && gateResult.currentUsage !== undefined" class="text-sm">
              <div class="flex justify-between mb-2">
                <span class="text-gray-600 dark:text-gray-400">Usage</span>
                <span class="font-medium">
                  {{ gateResult.currentUsage }} / {{ gateResult.limitValue || '∞' }}
                </span>
              </div>
              <UProgress 
            ...
            <!-- Grace window info -->
            <div v-if="gateResult.graceWindowAvailable" class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
...
const checkCapability = async () => {
  try {
    const result = await canUseCapability(props.capabilityKey, props.requestedUsage)
...
watch([() => props.capabilityKey, () => props.requestedUsage], checkCapability, { immediate: true })
