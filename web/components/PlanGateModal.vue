      ...
      <UCard>
          ...
          >
            <div class="flex items-start">
              <UIcon
                ...
                class="w-5 h-5 text-orange-500 mt-0.5 mr-3"
              />
              <div>
                <p
          ...
          <div v-if="showUsageInfo" class="space-y-3">
            <div class="flex justify-between text-sm">
              ...
              <span class="font-medium"
                ...
                {{ error?.limit_value || '∞' }}</span
              >
            </div>

            <UProgress
              :value="
                error?.current_usage && error?.limit_value
                  ? (error.current_usage / error.limit_value) * 100
                  : 0
              "
            ...
            <div class="flex items-start">
              ...
              <div>
                ...
                <p class="text-sm text-blue-700 dark:text-blue-300 mt-1">
                  You have {{ getGraceWindowTimeRemaining() }} to upgrade your
                  plan.
                </p>
              </div>
            </div>
            ...
            <div class="flex items-start">
              ...
              <div>
                ...
                <p class="text-sm text-green-700 dark:text-green-300 mt-1">
                  Upgrade to {{ suggestedPlan }} for unlimited access to this
                  feature.
                </p>
              </div>
            </div>
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <UButton color="primary" @click="handleUpgrade" :loading="loading">
              {{ upgradeText }}
            </UButton>
          </div>
        </template>
...
// Computed properties
const hasGraceWindow = computed(
  () => props.error?.grace_window_available || false
);
...
const getProgressColor = () => {
  if (!props.error?.current_usage || !props.error?.limit_value)
    return 'primary';

  const percentage =
    (props.error.current_usage / props.error.limit_value) * 100;
