...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    try {
      ...
      const effective = {
        currency_code: userPrefs?.currency_code || teamPrefs?.currency_code || 'USD',
        unit_family: userPrefs?.unit_family || teamPrefs?.unit_family || 'metric',
        date_format: userPrefs?.date_format || teamPrefs?.date_format || 'MM/DD/YYYY',
        time_format: userPrefs?.time_format || teamPrefs?.time_format || '12h',
        timezone_id: userPrefs?.timezone_id || teamPrefs?.timezone_id,
        number_format: userPrefs?.number_format || teamPrefs?.number_format || 'en-US'
      }
