  ...
  if (method === 'POST') {
    ...
    try {
      ...
      if (existingGraceWindow) {
        throw createError({
          statusCode: 409,
          statusMessage: 'Active grace window already exists for this capability'
        })
      }

      const startedAt = new Date()
      const expiresAt = new Date(startedAt.getTime() + validatedData.grace_period_days * 24 * 60 * 60 * 1000)
