  ...
  switch (errorType) {
    case PlanLimitErrorType.CAPABILITY_EXCEEDED:
      return {
        ...
        status: 429,
        detail:
          'The usage limit for this capability has been exceeded. Please upgrade your plan or wait for the next billing cycle.',
      };
    ...
    case PlanLimitErrorType.CAPABILITY_NOT_AVAILABLE:
      return {
        ...
        status: 403,
        detail:
          'This capability is not available in your current plan. Please upgrade to access this feature.',
      };
    ...
    case PlanLimitErrorType.GRACE_WINDOW_EXPIRED:
      return {
        ...
        status: 429,
        detail:
          'The grace window for this capability has expired. Please upgrade your plan to continue using this feature.',
      };
    ...
    case PlanLimitErrorType.PLAN_LIMIT_REACHED:
      return {
        ...
        status: 429,
        detail:
          'You have reached the maximum usage limit for your current plan. Please upgrade to continue.',
      };
    ...
    case PlanLimitErrorType.UPGRADE_REQUIRED:
      return {
        ...
        status: 402,
        detail:
          'A plan upgrade is required to access this feature or continue using this capability.',
      };
...
export const checkPlanLimits = async (
  ...
  try {
    ...
    const capability = capabilities?.find(
      (c: any) => c.capability_key === capabilityKey
    );
    ...
    if (!capability) {
      return {
        ...
        error: createPlanLimitError(
          PlanLimitErrorType.CAPABILITY_NOT_AVAILABLE,
          {
    ...
    if (
      ...
      if (capability.limit_value && totalUsage > capability.limit_value) {
        ...
        if (availableGraceWindow) {
          ...
          errorDetails.grace_window_expires_at =
            availableGraceWindow.expires_at;
        }

        return {
          allowed: false,
          error: createPlanLimitError(
            PlanLimitErrorType.CAPABILITY_EXCEEDED,
            errorDetails
          ),
        };
      }
...
export const planLimitMiddleware = async (
  ...
  const { allowed, error } = await checkPlanLimits(
    supabase,
    teamId,
    capabilityKey,
    requestedUsage
...
export const getSuggestedPlan = async (
  ...
  try {
    ...
    const sortedPlans = plans?.sort((a: any, b: any) => {
      const aLimit =
        a.plan_capabilities.find((c: any) => c.capability_key === capabilityKey)
          ?.limit_value || 0;
      const bLimit =
        b.plan_capabilities.find((c: any) => c.capability_key === capabilityKey)
          ?.limit_value || 0;
