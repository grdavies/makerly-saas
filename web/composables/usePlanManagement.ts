...
export const usePlanManagement = () => {
  ...
  const getRecommendedPlan = () => {
    ...
    const currentPlan = availablePlans.value.find(
      p => p.id === currentSubscription.value?.plan_id
    );
    if (!currentPlan) return null;

    // Find next plan in sort order
    const currentIndex = availablePlans.value.findIndex(
  ...
  const initialize = async () => {
    await Promise.all([loadAvailablePlans(), loadCurrentSubscription()]);
  };
