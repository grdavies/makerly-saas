...
export const useRBAC = () => {
  ...
  const { currentRole, currentTeamMember, isSuperAdmin, isTeamAdmin } = useAuth()
  ...
  const can = async (action: string, resource: string, location?: string): Promise<boolean> => {
    return hasPermission(action, resource, location)
  }

  // Check if user can create something
  const canCreate = async (resource: string, location?: string): Promise<boolean> => {
    return hasPermission('create', resource, location)
  }

  // Check if user can read something
  const canRead = async (resource: string, location?: string): Promise<boolean> => {
    return hasPermission('read', resource, location)
  }

  // Check if user can update something
  const canUpdate = async (resource: string, location?: string): Promise<boolean> => {
    return hasPermission('update', resource, location)
  }

  // Check if user can delete something
  const canDelete = async (resource: string, location?: string): Promise<boolean> => {
  ...
  const canManagePermissions = async (): Promise<boolean> => {
    return canCreate('permission', 'system') && canUpdate('permission', 'system')
  }
  ...
  const getResourcePermissions = async (resource: string, location?: string) => {
