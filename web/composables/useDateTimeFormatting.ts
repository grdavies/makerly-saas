...
export const useDateTimeFormatting = () => {
  ...
  const formatDate = (
    ...
    const dateFormat =
      options.dateFormat ||
      effectivePreferences.value?.date_format ||
      'MM/DD/YYYY';
    const locale =
      options.locale || effectivePreferences.value?.number_format || 'en-US';
    const timezone =
      options.timezone || effectivePreferences.value?.timezone_id;
  ...
  const formatTime = (
    ...
    const dateObj = typeof date === 'string' ? new Date(date) : date;
    const timeFormat =
      options.timeFormat || effectivePreferences.value?.time_format || '12h';
    const locale =
      options.locale || effectivePreferences.value?.number_format || 'en-US';
    const timezone =
      options.timezone || effectivePreferences.value?.timezone_id;
  ...
  const getTimezoneOptions = (): Array<{
    value: string;
    label: string;
    offset: string;
  }> => {
