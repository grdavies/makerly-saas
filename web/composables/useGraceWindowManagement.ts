...
export const useGraceWindowManagement = () => {
  ...
  const createGraceWindow = async (
    ...
    try {
      ...
      const expiresAt = new Date(
        startedAt.getTime() + gracePeriodDays * 24 * 60 * 60 * 1000
      );
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to create grace window';
  ...
  const useGraceWindow = async (graceWindowId: string) => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to use grace window';
  ...
  const expireGraceWindow = async (graceWindowId: string) => {
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to expire grace window';
  ...
  const loadGraceWindows = async () => {
    ...
    try {
      ...
      activeGraceWindows.value =
        data?.filter(gw => gw.status === 'active') || [];
    } catch (err) {
      error.value =
        err instanceof Error ? err.message : 'Failed to load grace windows';
  ...
  const canUseGraceWindow = (capabilityKey: string): boolean => {
    ...
    return (
      !isGraceWindowExpired(graceWindow) && graceWindow.status === 'active'
    );
  };
