...
export const use2FA = () => {
  ...
  const loadFactors = async () => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to load MFA factors';
    } finally {
      loading.value = false;
    }
  };
  ...
  const enrollTOTP = async (friendlyName: string = 'Authenticator App') => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to enroll TOTP';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const verifyTOTP = async (factorId: string, code: string) => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to verify TOTP';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const unenrollFactor = async (factorId: string) => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to unenroll factor';
      throw err;
    } finally {
      loading.value = false;
    }
