...
export const useUsageTracking = () => {
  ...
  const trackUsage = async (
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to track usage';
      ...
      for (const capability of capabilities || []) {
        if (
          ...
          summary.push({
            ...
            is_within_limits:
              currentUsage < (capability.limit_value || Infinity),
          });
        }
      }
  ...
  const loadUsageRecords = async (capabilityKey?: string) => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : 'Failed to load usage records';
  ...
  const getUsagePercentage = (capabilityKey: string): number => {
    const summary = usageSummary.value.find(
      s => s.capability_key === capabilityKey
    );
