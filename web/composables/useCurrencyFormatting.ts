...
export const useCurrencyFormatting = () => {
  ...
  const formatCurrency = (
    ...
    const currency =
      options.currency || effectivePreferences.value?.currency_code || 'USD';
    const locale =
      options.locale || effectivePreferences.value?.number_format || 'en-US';
    ...
    const formatOptions: Intl.NumberFormatOptions = {
      ...
      minimumFractionDigits:
        options.minimumFractionDigits ?? currencyInfo?.decimal_places ?? 2,
      maximumFractionDigits:
        options.maximumFractionDigits ?? currencyInfo?.decimal_places ?? 2,
  ...
  const formatCurrencyWithSymbol = (
    ...
    const currencyInfo = getCurrency(
      effectivePreferences.value?.currency_code || 'USD'
    );
    ...
    try {
      ...
      if (cachedRate) {
        const hoursSinceUpdate =
          (Date.now() - new Date(cachedRate.updated_at).getTime()) /
          (1000 * 60 * 60);
