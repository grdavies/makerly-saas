...
export const usePlanGates = () => {
  ...
  const { trackUsage, getUsage, isWithinLimits, usageSummary } = useUsageTracking()
  const { 
    hasGraceWindow, 
    canUseGraceWindow, 
  ...
  const canUseCapability = async (
    ...
    try {
      ...
      const capability = capabilities.value.find(c => c.capability_key === capabilityKey)
      ...
      if (capability.capability_type === 'numeric' || capability.capability_type === 'metered') {
        ...
        if (capability.limit_value && totalUsage > capability.limit_value) {
          ...
          if (graceWindowStatus.hasGraceWindow && !graceWindowStatus.isExpired) {
            return {
              allowed: true,
              currentUsage,
              limitValue: capability.limit_value,
              usagePercentage: Math.round((currentUsage / capability.limit_value) * 100),
  ...
  const useCapability = async (
    ...
    if (!options.skipTracking) {
      try {
        await trackUsage(capabilityKey, value, { window_type: windowType, metadata: options.metadata })
      } catch (err) {
  ...
  const getCapability = (capabilityKey: string): PlanCapability | null => {
    return capabilities.value.find(c => c.capability_key === capabilityKey) || null
  }
  ...
  const getCapabilityUsage = async (
    ...
    return {
      ...
      usage_percentage: capability?.limit_value 
        ? Math.round((currentUsage / capability.limit_value) * 100)
        : 0,
      is_within_limits: capability?.limit_value ? currentUsage < capability.limit_value : true,
  ...
  const requestGraceWindow = async (
    ...
    try {
      await createGraceWindow(capabilityKey, { grace_period_days: gracePeriodDays })
      return true
    } catch (err) {
  ...
  const loadPlanInfo = async () => {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to load plan information'
  ...
  const getCapabilitiesNeedingAttention = () => {
    return usageSummary.value.filter(summary => 
      summary.percentage_used >= 80 || !summary.is_within_limits
    )
  }
