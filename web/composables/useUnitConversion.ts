...
export const useUnitConversion = () => {
  ...
  const getUnitFamilyByName = (name: string): UnitFamily | undefined => {
    return unitFamilies.value.find(
  ...
  const getBaseUnit = (familyId: string): Unit | undefined => {
    return units.value.find(
      unit => unit.family_id === familyId && unit.is_base_unit
    );
  };
  ...
  const convertToPreferred = (
    ...
    if (!preferredBaseUnit)
      throw new Error('No base unit found for preferred family');
  ...
  const getUnitOptions = (
    familyId: string
  ): Array<{ value: string; label: string; symbol: string }> => {
  ...
  const getUnitFamilyOptions = (): Array<{
    value: UnitFamily;
    label: string;
  }> => {
  ...
  const convertTemperature = (
    ...
    if (!converter) {
      throw new Error(
        `Cannot convert temperature from ${fromUnit} to ${toUnit}`
      );
    }
